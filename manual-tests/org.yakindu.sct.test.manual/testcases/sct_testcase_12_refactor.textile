...
| 12.5| Fold Incoming Actions | Select LightOn state  <p> Right click on the state <p> Select Fold Incoming Actions | the action is shown in in the state LightOn and only the trigger is shown in the transitions  | %{color:red}open% |
| 12.5.1 | Simulate | Run simulation | Behavior should be the same as before re-factoring | %{color:red}open% |
| 12.6| Unfold Entry Actions | Select LightOn state  <p> Right click on the state <p> Select Unfold Entry Actions | the action is shown behind the trigger again | %{color:red}open% |
| 12.6.1 | Simulate | Run simulation | Behavior should be the same as before re-factoring | %{color:red}open% |
| 12.7| Fold Outgoing Actions | Select LightOff state  <p> Right click on the state <p> Select Fold Outgoing Actions | the action is shown in in the state LightOff and only the trigger is shown in the transitions  | %{color:red}open% |
| 12.7.1 | Simulate | Run simulation | Behavior should be the same as before re-factoring | %{color:red}open% |
| 12.8| Unfold Exit Actions | Select LightOFF state  <p> Right click on the state <p> Select Unfold Exit Actions | the action is shown behind the trigger again | %{color:red}open% |
| 12.8.1 | Simulate | Run simulation | Behavior should be the same as before re-factoring | %{color:red}open% |
