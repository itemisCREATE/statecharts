...
h4. Interfaces, Variable and Event access
...
h4. Optional header file for timing service support and operation callbacks

If the statechart uses timing or external operations an additional header file with the name pattern <statechart name>Required.h is generated. Because the traffic light example uses timing it contains such a header file:
