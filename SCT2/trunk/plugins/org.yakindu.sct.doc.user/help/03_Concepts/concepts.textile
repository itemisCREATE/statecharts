...
In the following the elements of statecharts that are used in the YAKINDU
SCT 2 editor are described. The meta model of the YAKINDU SCT 2 is the model of finite state machines. It is based on the view of a system that is defined by a finite number of states. The behavior of that system is based on the current states. A current state is determined by the history of the state machine. Very important are the models for state machines by Mealy and Moore. Mealy state machines associate actions with transitions. Moore machines associate actions with states (entry, do, exit).

The YAKINDU SCT 2 meta model is designed similar to the UML state chart meta model with the following differences

* self contained with interfaces well defined by events and variables
* core execution semantics are cycle driven, not event driven
* * this allows processing concurrent events
...
A transition is the transfer of one state to another. They are diagrammed as arrows and can carry events and actions but must not. 

Events are written in SText. Please refer to the documentation section Events for more details. For more details on Actions refer to the chapter Actions.
...
Initial and final states are pseudo states, because the state chart does not rest on them. Pseudostates express characteristics that are hardly to express by simple states. 
...
Choice is a decision node. It is also a pseudo state. It can be used to model a conditional path. Choice nodes divide a transition into multiple parts. 
...
A composite state is a state that is composed of other state machines. These are also organized in regions. YAKINDU knows three kinds of composite states: Composite states, orthogonal state and submachine states.

Composite states contain other state machine branches.

Orthogonal states ???

Submachine states may contain complete state machines. Here the user can chose another state machine to be included. 
