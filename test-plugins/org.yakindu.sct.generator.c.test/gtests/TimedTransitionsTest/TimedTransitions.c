...
{
	sc_boolean result;
	if (handle->stateConfVector[0] != TimedTransitions_last_state)
	{
		result =  bool_true;
	}
	else
...
sc_boolean timedTransitions_isStateActive(const TimedTransitions* handle, TimedTransitionsStates state)
{
	...
	{
		case TimedTransitions_main_region_Start :
			result = (sc_boolean) (handle->stateConfVector[0] == TimedTransitions_main_region_Start
			);
			break;
		case TimedTransitions_main_region_End :
			result = (sc_boolean) (handle->stateConfVector[0] == TimedTransitions_main_region_End
			);
			break;
		default:
			result = bool_false;
