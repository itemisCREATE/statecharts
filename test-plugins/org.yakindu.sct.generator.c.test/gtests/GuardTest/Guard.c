...
{
	sc_boolean result;
	if (handle->stateConfVector[0] != Guard_last_state)
	{
		result =  bool_true;
	}
	else
...
sc_boolean guard_isStateActive(const Guard* handle, GuardStates state)
{
	...
	{
		case Guard_main_region_A :
			result = (sc_boolean) (handle->stateConfVector[0] == Guard_main_region_A
			);
			break;
		case Guard_main_region_B :
			result = (sc_boolean) (handle->stateConfVector[0] == Guard_main_region_B
			);
			break;
		default:
			result = bool_false;
