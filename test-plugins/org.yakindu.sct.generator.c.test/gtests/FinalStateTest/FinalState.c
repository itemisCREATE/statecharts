...
{
	sc_boolean result;
	if (handle->stateConfVector[0] != FinalState_last_state)
	{
		result =  bool_true;
	}
	else
...
sc_boolean finalState_isStateActive(const FinalState* handle, FinalStateStates state)
{
	...
	{
		case FinalState_main_region_myState :
			result = (sc_boolean) (handle->stateConfVector[0] == FinalState_main_region_myState
			);
			break;
		case FinalState_main_region__final_ :
			result = (sc_boolean) (handle->stateConfVector[0] == FinalState_main_region__final_
			);
			break;
		default:
			result = bool_false;
