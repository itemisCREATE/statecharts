...
{
	sc_boolean result;
	if (handle->stateConfVector[0] != TriggerGuardExpressions_last_state)
	{
		result =  bool_true;
	}
	else
...
sc_boolean triggerGuardExpressions_isStateActive(const TriggerGuardExpressions* handle, TriggerGuardExpressionsStates state)
{
	...
	{
		case TriggerGuardExpressions_main_region_A :
			result = (sc_boolean) (handle->stateConfVector[0] == TriggerGuardExpressions_main_region_A
			);
			break;
		case TriggerGuardExpressions_main_region_B :
			result = (sc_boolean) (handle->stateConfVector[0] == TriggerGuardExpressions_main_region_B
			);
			break;
		default:
			result = bool_false;
