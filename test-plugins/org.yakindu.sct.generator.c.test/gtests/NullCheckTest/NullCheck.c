...
{
	sc_boolean result;
	if (handle->stateConfVector[0] != NullCheck_last_state)
	{
		result =  bool_true;
	}
	else
...
sc_boolean nullCheck_isStateActive(const NullCheck* handle, NullCheckStates state)
{
	...
	{
		case NullCheck_main_region_A :
			result = (sc_boolean) (handle->stateConfVector[0] == NullCheck_main_region_A
			);
			break;
		case NullCheck_main_region_B :
			result = (sc_boolean) (handle->stateConfVector[0] == NullCheck_main_region_B
			);
			break;
		default:
			result = bool_false;
