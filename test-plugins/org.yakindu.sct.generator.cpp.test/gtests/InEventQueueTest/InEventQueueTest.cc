...
TEST(InEventQueue, InEventQueue) {
	...
	EXPECT_TRUE( machine->isStateActive(InEventQueue::main_region_A) );


	nextEvent = 'f';
	machine->getDefaultSCI()->raise_e();
	EXPECT_TRUE( machine->isStateActive(InEventQueue::main_region_B) );


	nextEvent = 'g';
	machine->getDefaultSCI()->raise_e();
	EXPECT_TRUE( machine->isStateActive(InEventQueue::main_region_C) );
