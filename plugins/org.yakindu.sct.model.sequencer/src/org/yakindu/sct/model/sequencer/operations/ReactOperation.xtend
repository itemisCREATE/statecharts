...
import org.yakindu.sct.model.sequencer.expressions.ReactionToExpression
...
class ReactOperation {
	...
	def dispatch defineReactMethod(RegularState state) {
		...
		val parentNode = if (state.parentState !== null) state.parentState else state.statechart
