...
public class FunctionConstructor {
	...
	public Function construct(FunctionDefinition functionDefinition, DiagnosticChain diagnostics) {
		...
		for (EquationDefinition equationDefinition : functionDefinition.getEquations()) {
			...
			setupEquationParts(lhs, variableReferences, diagnostics);
			...
			setupEquationParts(rhs, variableReferences, diagnostics);
		}
	...
	protected void setupEquationParts(EquationSide equationSide, VariableReferenceMap variableReferences, DiagnosticChain diagnostics) {
		for (TreeIterator it = new TreeIterator(equationSide.getExpression(), true); it.hasNext();) {
	...
	protected VariableReferenceKind getVariableReferenceKind(FunctionDefinition functionDefinition, String name) {
	...
	protected Expression getStepExpression(FeatureCall featureCall, DiagnosticChain diagnostics) {
