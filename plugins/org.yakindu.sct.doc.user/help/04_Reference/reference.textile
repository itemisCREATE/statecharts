...
Please refer to the description of the "UML state machine":http://en.wikipedia.org/wiki/UML_state_machine for more details.
...
It is an *optinal* feature and has the following parameters:
...
Do we need some equivalent to the C section here?
...
To execute the C example, run the file _org_yakindu_sct_examples_c_trafficlight_ as "Local C/C++ application" from the Eclipse _Run As_ context menu.
...
>>> In Java we have this chapter. What about a C++ equivalent? <<<
