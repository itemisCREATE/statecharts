...
!(standard-image full)images/tuto_light_switch_330_statechart_simulator_state_on.png(Interactive light switch state machine

p=. Interactive light switch state machine simulation
...
# The light switch state machine is defined in the file _light_switch.sct_. Double-click on this file to open the model.
...
h2(standard #oss_generating-statechart-code). Generating code from state machine

You have developed and tested your state machine model, and now you need it as a C, C++, or Java implementation? YAKINDU Statechart Tools can create it for you.

As an example, we will take the light switch state machine and generate Java source code from it. Please proceed as follows:
...
This tutorial introduces YAKINDU Statechart Tools (SCT) which provides an integrated modeling environment for the specification and development of reactive, event-driven systems based on the concept of finite state machines (fsm) or statecharts. It is an easy-to-use tool featuring sophisticated graphical state model editing, validation and simulation of state machines as well as code generation for C, C&#43;&#43;, or Java.

This tutorial you will teach how to

* create a new state machine model, and
* execute it using the simulation engine.

Please note that this tutorial will not explain state machines (fsm) in general, so you should familiarize yourself with the basic concepts of state machines first, see "UML state machine":http://en.wikipedia.org/wiki/UML_state_machine.
...
h2(#the-light-switch-example). The light switch state machine example
...
When modeling the light switch example as a state machine, we can disregard everything (even the bulb) except for the switch. The behavior of the switch is very simple:
...
h2(#modeling-the-light-switch-as-a-statemachine). Modeling the light switch as a state machine

Now let's model the light switch as a state machine (fsm) using YAKINDU Statechart Tools. This involves the following steps:

# Creating an Eclipse project. It will serve as a container for the model.
...
Now that we have created the Eclipse project, we can establish the state machine model of the light switch. It is good practice to use a separate directory for models, so let's create a new folder called _model_.
...
h3(#starting-with-an-empty-state-machine-model). Starting with an empty state machine model

h4(#using-the-yakindu-statechart-wizard). Using the YAKINDU Statechart wizard

Now we are going to create an empty state machine model in the _model_ folder. Right-click on the _model_ folder and select _New → Other_ in the context menu:
...
Since there's a dedicated perspective for state machines, Eclipse asks you whether you want to change to it now or not:
...
An empty state machine is created and displayed in the *statechart editor* in the middle of the Eclipse workbench. In the screenshot below the statechart editor is marked by a red rectangle. Subsequently, we will use the statechart editor to graphically develop the light switch state machine.
...
Actually the new state machine isn't really empty. It already contains the initial state (a small filled black circle), a "normal" state, and a transition leading from the former to the latter.
...
This error marker exemplifies YAKINDU Statechart Tools' model validation capabilities. Statechart validation includes syntactical and semantical checks of the whole state machine. For example, a state machine is checked for unreachable states, dead ends, or references to unknown events. These validation constraints are checked during editing. In case any constraints are violated, error or warning markers are attached to the faulty model elements. Thus the user receives direct and immediate feedback on the validation state of his statechart (state machine).
...
As we have seen, the initial state machine is erroneous insofar as the state does not have a name. So let's fix that.
...
table(scedit).
|<. Double-click on the string @<name>@ in the state object. The string turns into a text input field with @<name>@ being highlighted:|<. !(standard-image)images/tuto_light_switch_230_statechart_editor_change_state_name_01.png!|
...
With the *Off* state only, the light switch state machine isn't complete yet. We also need an *On* state, and we going to create it now.
...
h2(#simulating-the-light-switch-model). Simulating the light switch state machine
...
The complete state machine model is shown below:

!(standard-image)images/tuto_callhandling_model.png(The CallHandling state machine model)!

p=. The CallHandling state machine model
...
h3(#creating-the-state-machine-model). Creating the state machine model
...
Since we are going to generate Java code, we should use a Java project to host the state machine model. Use _File → New → Java Project_ and follow the _New Java Project_ wizard to create one.
...
!(standard-image)images/tuto_callhandling_example_final.png(The CallHandling state machine modeled in the statechart editor)!

p=. The CallHandling state machine modeled in the statechart editor
...
A state machine reference may contain various configuration features. You will learn more about them later.
...
The generator model is executed by a so-called Eclipse _builder_. That is, as long as the _Project → Build Automatically_ menu item is checked (which it is by default), the artifacts are generated automatically with each modification of the state maschine model or of the generator model.
...
# Name it _CallHandlingClient_.<br/>!(standard-image)images/tuto_callhandling_300_java_integration_create_new_class.png!
# Click _Finish_.
