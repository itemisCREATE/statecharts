...
h3(#oss_which-licensing-rules-do-apply). Which licensing rules do apply to YAKINDU Statechart Tools?
...
Statecharts can get rather big and complex. *"Composite states":#composite-state* are a way to reduce complexity and thus make statecharts easier to create, comprehend and maintain. A composite state comprises a state machine of its own within a "region":#region. The states belonging to such a nested state machine are called *substates*. *"Orthogonal states":#orthogonal-states* are a generalization of composite states, comprising two or more independend state machines in separate regions that are executed in parallel.
...
An introduction to simulation is given in section "&quot;Simulating the light switch model&quot;":#simulating-the-light-switch-model. A future version of this documentation will provide a comprehensive documentation of all simulation features. For now, we are just giving you the overview below. Although it intends to be complete in the sense that it lists all simulation features, the overview does not go into any detailed usage instructions. Section "&quot;Simulating the light switch model&quot;":#simulating-the-light-switch-model will be helpful. Finding out everything not mentioned there is left as an exercise to the reader for now.
...
For configuring the code generation process, YAKINDU Statechart Tools uses a textual generator model called *SGen*. It can be created either by using the _YAKINDU Statechart Generator Model_ wizard or by manually creating a text file with the @.sgen@ filename extension. This textual model specifies generator features, e.&nbsp;g. the outlet path. "Figure &quot;SGen generator model with default values&quot;":#oss_fig_sgen-generator-model-with-default-values shows a sample configuration for the Java code generator.
...
p(#oss_fig_sgen-generator-model-with-default-values). 
...
# Choose the desired generator, e.&nbsp;g. _YAKINDU SCT Java Code Generator_.
...
# In the project explorer view, right-click at the @.sgen@ file containing the generator model. The context menu opens.
...
h4(#oss_responsibilities-of-generated-code). Responsibilities of generated code
...
h4(#oss_cycle-based-execution). Cycle-based execution
...
h4(#oss_thread-safe-execution). Thread-safe execution
...
h4(#oss_runnablewrapper). RunnableWrapper
...
Although YAKINDU Statechart Tools are shipped with powerful code generators for C, C++ and Java, it may be necessary to create custom code generators to support a variety of use cases. One reason for a custom code generator could be to support additional programming languages – in this case we would be happy to receive a contribution – or to generate code for an existing framework. Custom code generators can be either written in Java, "Xtend":http://www.eclipse.org/xtend/, or "Xpand":http://wiki.eclipse.org/Xpand.

h3. Prerequisites

Implementing a custom code generator is no trivial task. Before you get started, you should have a basic understanding of the "Eclipse Modeling Framework (EMF)":https://www.eclipse.org/modeling/emf/. YAKINDU Statechart Tools are using EMF to structure its data model. Furthermore, we highly recommend to use "Xtend":http://www.eclipse.org/xtend/ as your code generator's template language, although plain old Java is still supported. Xtend provides some great features like "template expressions":http://www.eclipse.org/xtend/documentation/203_xtend_expressions.html#templates, "lambda expressions":http://www.eclipse.org/xtend/documentation/203_xtend_expressions.html#lambdas, or "polymorphic method invocation":http://www.eclipse.org/xtend/documentation/202_xtend_classes_members.html#polymorphic-dispatch, boosting readability and productivity.

h3. Creating a new code generator project

Creating custom code generators is a first-level concept in YAKINDU Statechart Tools. It is not required to set up a developer workspace with the YAKINDU Statechart Tools source code and to start a new Eclipse runtime to test your generator changes. Instead, you can develop and test your custom generator at runtime.

h4(#oss_writing-a-custom-code-generator-with-xtend). Writing a custom code generator with Xtend

p(#oss_fig_creating-an-xtend-generator-project). 

!(standard-image)images/custom_generator_010_create_generator_project.png(Creating an Xtend generator project)!

p=. Creating an Xtend generator project

To set up a new Xtend generator project, proceed as follows:

# In the main menu, select _File → New → Other… → YAKINDU SCT → Xtend2/Java Generator Project_.
# Click _Next >_. The _New YAKINDU Workspace Generator Project_ wizard appears, see "figure &quot;Creating an Xtend generator project&quot;":#oss_fig_creating-an-xtend-generator-project.
# In the respective wizard input fields, specify the _project name_ of the project to be created (here: *MyGenerator*) and the fully-qualified name of the _generator class_ that will be created in the new project (here: *com.example.MyGenerator*).
# Check the _Use Xtend_ checkbox. The generator class will initially be created as an "Xtend":http://www.eclipse.org/xtend/ class. If you don't check _Use Xtend_, the generator class will be pure Java.
# If you plan to export your custom generator as an Eclipse plugin in order to deploy it to various YAKINDU Statechart Tools installations, check the _Configure plugin for export_ checkbox. All required extension point registrations will be added to the new project.
## Specify _Generator ID_, _Generator name_ and _Generator description_. These information are used to make your generator accessible from a generator model.
## Check the _Create Feature Library_ check box if you want to establish custom generator features for your code generator.
# Click _Finish_. A new project is created in your workspace. All required plugin dependencies and extension points are registered and you can start to write your code generator. The file _CustomGenerator.xtend_ – see "figure &quot;Default Xtend generator project&quot;":#oss_fig_default-xtend-generator-project – contains the default code generator that simply writes the names of the statechart and its states to the target file. You can use it as the starting point for your own code generator.

###. FIXME: The above description referred to the ExecutionFlow meta model. What is this and where is it documented?

p(#oss_fig_default-xtend-generator-project). 

!(standard-image)images/custom_generator_020_default_generator_project.png(Default Xtend generator project)!

p=. Default Xtend generator project

h3(#ExecutingacustomXtend2Javacodegenerator). Executing a custom Xtend code generator

In order to execute your custom code generator you need two different models:
* a statechart model,
* a generator model.

The generator model must have a generator ID of @yakindu::generic@. You can either use the _YAKINDU Generator Model_ wizard to establish the generator model, or you can simply create a text file with the filename extension @.sgen@. Its content might look like in the following example:

bc(prettyprint).. 
GeneratorModel for yakindu::generic {

	statechart MyExample {

		feature Outlet {
			targetProject = "MyGenerator"
			targetFolder = "src-gen"
			libraryTargetFolder = "src"
		}

		feature Generator {
			generatorProject = "MyGenerator"
			generatorClass = "org.example.MyGenerator"
		}
	}
}

p. As you may have noticed, the generator model for the @yakindu::generic@ generator has a feature called _Generator_, which is described in section "&quot;Generator feature&quot;":#oss_generator-feature.

Right-click at the @.sgen@ file and select _Generate Statechart Artifacts_ from the context menu to execute the generator, creating a file _src-gen/MyStatechart.txt_ with the following content:

bc(prettyprint). 
The name of the state machine is 'MyExample'
The state machine has the following states:
main_region.A
main_region.B

Congratulations, you successfully created a custom generator project!

Now add

bc. 
The state machine contains «flow.states.size» states.

to the _CustomGenerator.xtend_ file and regenerate. This results in _MyStatechart.txt_ being updated immediately. This is a very powerful feature of YAKINDU Statechart Tools. You can develop your code generator at runtime with zero turnaround time. Just select _Generate Statechart Artifacts_ and see the results.

==<!-- Start sgen_feature_generator -->==

h4(#oss_generator-feature). Generator feature

The *Generator* feature allows to configure a custom code generator located in the workspace and written in Java or in another JVM language. It is a _required_ feature and has the following parameters:
...
h3. The SGraph meta model

The *SGraph meta model* defines the structural aspects of the statechart model and is similiar to the state machine model defined by the Unified Modeling Language (UML). "Figure &quot;Simplified SGraph meta model&quot;":#oss_fig_simplified-sgraph-meta-model shows a simplified version.

p(#oss_fig_simplified-sgraph-meta-model). 

!(standard-image)images/sgraph_simple.png(Simplified SGraph meta model)!

p=. Simplified SGraph meta model

* *Statechart* extends __CompositeElement__, therefore it contains 0..* __Regions__. It is the model's root element.
* *CompositeElement* is an abstract type containing __Regions__. Known subclasses are __Statechart__ and __State__.
* *Region* contains 1..* __Vertices__.
* *Vertex* is an abstract type representing nodes in the SGraph tree. A Vertex contains outgoing __Transitions__.
...
* *Choice* is a __Pseudostate__ with the additional attribute __kind__, determining whether the element has __static__ or __dynamic__ execution semantics.
