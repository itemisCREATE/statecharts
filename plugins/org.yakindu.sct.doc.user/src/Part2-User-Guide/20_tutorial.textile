...
# Click on _operate_. The light switch model's active state changes from *Off* to *On*. <p>!(standard-image)images/light_switch_330_statechart_simulator_state_on.png(Light switch simulation in "on" state)!</p>
...
# In the dialog, replace @default.sgen@ by @light_switch.sgen@, then click _Next >_. <p>!(standard-image)images/5min_120_naming_the_code_generator_model.png(Naming the code generator model)!</p>
# In the next dialog, check the checkbox left from _LightSwitch.sct_, then click _Finish_. <p>!(standard-image)images/5min_130_selecting_the_statechart_model.png(Selecting the statechart model)!</p>
# Java classes implementing the statechart model are generated. You'll find their source code in the folders _src_ and _src-gen_ within the _light_switch_ project. <p>!(standard-image)images/5min_140_generated_java_source_code.png(Generated Java source code)!</p>
