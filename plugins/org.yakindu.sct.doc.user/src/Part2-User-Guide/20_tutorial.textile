...
The five-minutes tutorial gives you a brief introduction into YAKINDU Statechart Tools by a simple example. It models a light switch, which has the states _on_ and _off_. Operating the switch repeatedly, turns the light on, off, back on again an so forth. You can simulate this behavior in a dynamic statechart model. Here's how this is looking like:

!(standard-image full)images/light_switch_330_statechart_simulator_state_on.png(Interactive light switch simulation)!

p=. Interactive light switch simulation

In the image above, the light switch is currently in the _on_ state. To toggle the light switch, the user clicks on _operate_ on the right-hand side of the window.

h2(#oss_running-the-light-switch-simulation). Running the light switch simulation

Now try this for yourself. We are assuming that you have installed and started YAKINDU Statechart Tools. If you don't, please see the "Installation Guide":../Part1-Installation-Guide/00_installation.html for details.

# If the "Welcome" page is still open, close it. !(standard-image small)images/5min_010_close_welcome.png(Closing the "Welcome" page)!
# Select _File → New → Example…_ in the main menu.
# In the dialog, select _YAKINDU SCT Examples → Light Switch Example_ and click _Next >_.
# In the following dialog, click _Finish_. The sample project _light_switch_ is created.
# In the project explorer on the left-hand side of the window, open the _light_switch_ project by clicking on the small triangle left from the project name ("light_switch"). !(standard-image small)images/5min_020_expand_project.png(Expand project)!
# Right-click on _LightSwitch.sct_ and select _Run As → Statechart Simulation_ in the context menu.
# The statechart simulation starts and shows the light switch model in the _off_ state. The rectangle named _Off_ is highlighted in red, meaning it is the active state. !(standard-image)images/light_switch_320_statechart_simulator_state_events.png(Light switch simulation in "off" state)!
# At the right side of the window, click on the small triangle left from _internal_. This unhides the _operate_ link.
# Click on _operate_. The light switch model's active state changes from _Off_ to _On_. !(standard-image)images/light_switch_330_statechart_simulator_state_on.png(Light switch simulation in "on" state)!
# Click on _operate_ again to switch off the light. Repeat as you see fit.

h2(standard #oss_turning-the-light-switch-statechart-into-java-code). Turning the light switch statechart into Java code

You have developed and tested your statechart model, and now you need it as a C, C++, or Java implementation? YAKINDU Statechart Tools can create it for you. In order to generate Java source code from the light switch example, proceed as follows:

# In the project explorer, right-click on _LightSwitch.sct_ and select _New → Code Generator Model_. !(standard-image)images/5min_110_new_code_generator_model.png(Creating a new code generator model)!
# In the dialog, replace @default.sgen@ by @light_switch.sgen@, then click _Next >_. !(standard-image)images/5min_120_naming_the_code_generator_model.png(Naming the code generator model)!
# In the dialog, check the checkbox left from _LightSwitch.sct_, then click _Finish_. !(standard-image)images/5min_130_selecting_the_statechart_model.png(Selecting the statechart model)!
# Java classes implementing the statechart model are generated. You'll find their source code in the folders _src_ and _src-gen_ within the _light_switch_ project. !(standard-image)images/5min_140_generated_java_source_code.png(Generated Java source code)!
...
h3(#the-behaviour-of-a-light-switch). The behaviour of a light switch
