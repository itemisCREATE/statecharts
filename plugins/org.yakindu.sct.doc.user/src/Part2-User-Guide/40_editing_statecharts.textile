...
* _Project Explorer_ (left): This view displays your workspace and therein contained projects, folders, and files. You can also use the _Project Explorer_ to inspect the internal structure of your statechart models.
...
* _Outline_ (right): This view is a bird's eye view on the opened statechart. It also indicates current the viewport for better orientation in large models.
...
** To remove a region from the canvas, select the region, then press @[Del]@ or select _Edit → Delete_ from the menu bar.
>>>>>>> master
* Zooming
...
=======
You can hide the palette by clicking the small triangle in the palette's title bar. Click the triangle again to make the palette reappear.
>>>>>>> master
