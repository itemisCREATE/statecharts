...
Here's the light switch example's statechart:
...
The statechart's definition section looks like this:
...
* In the cycle-based execution scheme, both events are handled at the same time, or, to be more exact, in the same run-to-completion step (RTS).
* In the event-driven execution scheme, the two events will be processed in the order you raised them. Each click on 
...
* The simulation view shows the _LightOn_ and the _LightOff_ events. Both events are labeled with a !(inlinemediaobject)images/advsim_symbol_event.png(Symbol: Event [not raised])! symbol, meaning the respective event is not raised.
