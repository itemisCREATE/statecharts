...
h1(#editing-statecharts). Editing statecharts (state machines)

h2(#creating-and-deleting-statecharts). Creating and deleting state machines

State machines are comprised in *statechart model files*. The filename extension of these files is @.sct@. Their internal format is "XML Metadata Interchange":https://en.wikipedia.org/wiki/XML_Metadata_Interchange or "XMI":https://en.wikipedia.org/wiki/XML_Metadata_Interchange, which is an XML language.

h3(#creating-a-statechart). Creating a state machine
...
h3(#copying-an-existing-statechart). Copying an existing state machine
...
h3(#deleting-a-statechart). Deleting a state machine
...
_SC Modeling_ is an Eclipse perspective supporting the modeling of state machines. The perspective defines the following views and their positions:
...
When building a state machine model step by step, you may come into a situation where you have defined several transitions having the same target state and share a common set of actions.
...
h2(#exporting-a-statechart-as-an-image-file). Exporting a state machine as an image file

A state machine can be saved as an image file as shown in the following steps:
