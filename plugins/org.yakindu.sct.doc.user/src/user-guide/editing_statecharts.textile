...
* top-level regions, which are directly placed on the "canvas":#canvas, and
...
h3(#editing-top-level-regions). Editing top-level regions

You can place a top-level region anywhere on the "canvas":#canvas, and you can change its size at will. To change the location or the size of a region, use the graphical editor. Drag the region to move it elsewhere. Use a selected region's handles to resize it.



h3(#editing_regions_in_orthogonal_states). Editing regions in orthogonal states
...
h3(#changing_the_name_of_a_region). Changing the name of a region
...
h4(#generator-model-filter). Generator model filter

Generator models can be executed automatically during the IDEs build. The build will normally execute all generator models that can be found in current workspace, what might be disruptive in some cases. To do a more fine grained configuration it is possible to filter particular generator models or entire folders out of the automatic build execution. To do so right-click on a project within your workspace > Properties > YAKINDU SCT > SGen Filter. Within the opened properties page you can simply add generator models or folders which have to be ignored during build execution.
