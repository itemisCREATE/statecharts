...
Eclipse uses the library GTK for drawing the user interface on Linux. Some users experience issues, especially when using the default version GTK3. If your installation of YAKINDU Statechart Tools does not perform as expected, is laggy, has drawing issues or looks weird, refer to "Configure YAKINDU Statechart Tools to use GTK2":#inst_linux_gtk2. If the welcome page looks broken and asks you to configure _webkitgtk_, refer to "Webkit GTK":#inst_linux_webkit_gtk.


h4(#inst_linux_gtk2). Configure YAKINDU Statechart Tools to use GTK2

_GTK+_ is one of two major graphics libraries used in the Linux world (the other one being _Qt_) and is the one used by the Eclipse project on which YAKINDU Statechart Tools is based. There are two different versions of GTK+: The newest one is often referred to as "GTK3", and the old one is named "GTK2" or just "GTK". Eclipse made the switch to GTK3 with the Mars release. While in general it looks and performs somewhat better, some users experience issues with it, especially concerning the performance or just drawing bugs, like wrong colors being used or widgets not displaying properly. If you experience anything like that, you should try to configure YAKINDU Statechart Tools (or the Eclipse instance you installed it into) to use GTK2, which is fairly easy to do.

(If you downloaded the standalone version of YAKINDU Statechart Tools, the application is called "SCT" and the .ini-file you'll have to edit is named "SCT.ini". If you installed it into an existing Eclipse installation, the names are "eclipse" and "eclipse.ini", respectively)

Navigate to the root installation folder of your installation, where either the application "eclipse" or "SCT" is located. There you will also find the "SCT.ini" or "eclipse.ini" configuration file, which you can open with a text editor of your choice. 

Note the line "-vmargs". Just *before* this line, paste those two new lines:
...
*Note*: Do *not* attempt to replace all of the contents of your SCT.ini file with the lines above. The specified version numbers have to match your installation and if you change them SCT will not start anymore.

To double-check that SCT now uses GTK2: just start it, navigate to _Help → About Yakindu SCT → Installation Details → Configuration_. You will get a bunch of text containing all kinds of internal variables. Not too far down, you will find _org.eclipse.swt.internal.gtk.version_, which should now have major version number 2, for example 2.24.30.
