...
If these code generators don't suit your needs, you are free to develop your very own "custom code generator":#generating_code_custom_code_generators.
...
h2(#generating_code_configuring_a_generator). Configuring a generator
...
h1(#generating_code_c_code_generator). C code generator

h2(#generating_code_c_code_generator_features). C code generator features

==<!-- Start sgen_feature_identifiersettings -->==

h3(#generating_code_c_identifiersettings_feature). IdentifierSettings feature
...
h2(#generating_code_cpp_code_generator_features). C++ code generator features

The C++ generator features are the same a the "C generator features":#generating_code_c_code_generator_features, plus the following ones.
...
h4(#generating_code_cpp_function_unsettimer). Function unsetTimer
...
h4(#generating_code_cpp_function_raisetimeevent). Function raiseTimeEvent
...
h3(#generating_code_cpp_accessing_interfaces_variables_and_events). Accessing interfaces, variables and events
...
h3(#generating_code_cpp_operation_callbacks). Operation callbacks
...
h2(#generating_code_java_code_generator_features). Java code generator features
...
h4(#generating_code_java_fundamental_statechart_methods). Fundamental statechart methods
...
h3(#generating_code_java_time_controlled_state_machines). Time-controlled state machines
...
h3(#generating_code_java_default_timer_implementation). Default timer implementation
...
h3(#generating_code_java_timer_service). Timer service
...
If suitable, an application can use the default timer service class _TimerService_, see "section &quot;Default timer implementation&quot;":#generating_code_java_default_timer_implementation for details.
...
h4(#generating_code_java_method_settimer). Method setTimer
...
h4(#generating_code_java_method_unsettimer). Method unsetTimer
...
h3(#generating_code_java_raising_time_events_on_a_state_machine). Raising time events on a state machine
...
h4(#generating_code_java_method_timeelapsed). Method timeElapsed
...
h3(#generating_code_java_runtime_service). Runtime service
...
h3(#generating_code_java_accessing_interfaces_variables_and_events). Accessing interfaces, variables and events
...
h3(#generating_code_java_interface_observers). Interface observers
...
h3(#generating_code_java_operation_callback). Operation callbacks
...
h3(#generating_code_java_integrating_generated_code). Integrating generated code
...
h3(#generating_code_java_simulating_operations_with_custom_java_code). Simulating operations with custom Java code
...
h1(#generating_code_custom_code_generators). Custom code generators
...
h2(#generating_code_creating_a_new_custom_code_generator_project). Creating a new custom code generator project
...
h3(#generating_code_executing_the_custom_code_generator). Executing the custom code generator
...
h3(#generating_code_sgen_features_for_custom_code_generators). SGen features for custom code generators

==<!-- Start sgen_feature_generator -->==

h4(#generating_code_sgen_generator_feature). Generator feature
...
h2(#generating_code_different_meta_models_for_different_use_cases). Different meta models for different use cases
