...
h3(#c-identifiersettings-feature). IdentifierSettings feature
...
h3(#c-tracing-feature). Tracing feature
...
h4(#c-fundamental-statechart-functions). Fundamental statechart functions
...
h4(#c-function-settimer). Function setTimer
...
h4(#c-function-unsettimer). Function unsetTimer
...
h4(#c-function-raisetimeevent). Function raiseTimeEvent

In order to notify the state machine about the occurence of a time event after a period of time has expired, the @raiseTimeEvent()@ function – defined in the header file of the state machine – needs to be called on the state machine. In the case of the traffic light example it is named @trafficLight_raiseTimeEvent(const TrafficLight* handle, sc_eventid evid)@ (in file _TrafficLight.h_).
...
Please note: To enable this feature, set the _RuntimeService_ parameter of *GeneralFeatures* to @true@, see "Java: General Features":#java-GeneralFeatures. 
...
Creating custom code generators is a built-in concept in YAKINDU Statechart Tools. You can develop custom code generators directly within your project workspace. You can choose between "Xtend2":http://www.eclipse.org/xtend/ (preferred) or simply Java for implementing the code generator templates. 
