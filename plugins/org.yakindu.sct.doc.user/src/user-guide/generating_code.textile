...
h1(#generating_code_generating-state-machine-code). Generating state machine code
...
Use "SCTUnit":../user-guide/sctunit.html in order to test whether your state machine is also correct in the sense that it does what it is intended to do. SCTUnit is available in YAKINDU Statechart Tools Professional Edition.
...
If these code generators don't suit your needs, you are free to develop your very own "custom code generator":#generating_code_custom-code-generators.




h2(#generating_code_general-concepts-of-the-generated-state-machine-code). General concepts of the generated state machine code

h3(#generating_code_generating_code_cycle-based-execution). Cycle-based execution
...
h3(#generating_code_generating_code_responsibilities-of-generated-code). Responsibilities of generated code
...
h3(#generating_code_generating_code_thread-safe-execution). Thread-safe execution

In order to circumvent the missing thread-safety of the _runCycle()_ method, the Java code generator has an option for generating a so-called runnable wrapper, providing event queueing and multi-threading support. For details, see section "&quot;RunnableWrapper feature&quot;":#generating_code_java_runnablewrapper_feature. It is easy to implement this as a general solution for Java, since the programming language provides corresponding standard features that by definition are available everywhere and can simply be used.
...
h2(#generating_code_configuring-a-generator). Configuring a generator
...
h3(#generating_code_outlet_feature). Outlet feature
...
h3(#generating_code_generating_code_licenseheader_feature). LicenseHeader feature
...
h3(#generating_code_generating_code_functioninlining_feature). FunctionInlining feature
...
h3(#generating_code_generating_code_debug_feature). Debug feature
...
h1(#generating_code_c_code-generator). C code generator

h2(#generating_code_c_generator_features). C generator features

==<!-- Start sgen_feature_identifiersettings -->==

h3(#generating_code_generating_code_c_identifiersettings_feature). IdentifierSettings feature
...
h3(#generating_code_c_tracing_feature). Tracing feature
...
h3(#generating_code_c_includes_feature). Includes feature
...
h2(#generating_code_c_specification-of-c-code). Specification of C code

In order to describe the API specifications of the code generated by the YAKINDU C generator, the explanations below are using the _Traffic Light_ sample state machine, see "figure &quot;The _Traffic Light_ sample statechart model&quot;":#generating_code_c_fig_the_traffic_light_sample_statechart_model. It models a pedestrian crossing with push-button operated traffic lights ("pelican crossing").

p(#generating_code_c_fig_the_traffic_light_sample_statechart_model). 
...
h3(#generating_code_c_generated-code-files). Generated code files
...
h4(#generating_code_c_fundamental-statechart-functions). Fundamental statechart functions
...
h4(#generating_code_c_accessing-variables-and-events). Accessing variables and events
...
h3(#generating_code_c_time-controlled-state-machines). Time-controlled state machines
...
h4(#generating_code_c_time-controlled-state-machines-example). Example
...
h4(#generating_code_c_function-settimer). Function setTimer
...
h4(#generating_code_c_function-unsettimer). Function unsetTimer
...
h4(#generating_code_c_function-raisetimeevent). Function raiseTimeEvent
...
h3(#generating_code_c_operation-callbacks). Operation callbacks
...
h3(#generating_code_c_integrating-generated-code). Integrating generated code
...
h1(#generating_code_cpp_code-generator). C++ code generator

h2(#generating_code_cpp_generator_features). C++ generator features

The C++ generator features are the same a the "C generator features":#generating_code_c_generator_features, plus the following ones.

==<!-- Start sgen_feature_generatoroptions -->==

h3(#generating_code_cpp_generatoroptions_feature). GeneratorOptions feature
...
h3(#generating_code_cpp_includes_feature). Includes feature
...
h2(#generating_code_cpp_specification-of-cpp-code). Specification of C++ code

In order to describe the API specifications of the code generated by the YAKINDU C++ generator, the explanations below are using the _Traffic Light_ sample state machine, see "figure &quot;The _Traffic Light_ sample statechart model&quot;":#generating_code_cpp_fig_the_traffic_light_sample_statechart_model. It models a pedestrian crossing with push-button operated traffic lights ("pelican crossing").

p(#generating_code_cpp_fig_the_traffic_light_sample_statechart_model). 
!(full)images/docu_TrafficLight.png(The _Traffic Light_ sample statechart model)!
...
h3(#generating_code_cpp_generated-code-files). Generated code files
...
h3(#generating_code_cpp_statemachine-class). Statemachine class
...
h3(#generating_code_cpp_abstract-class-statemachineinterface). Abstract class StatemachineInterface
...
h4(#generating_code_cpp_fundamental-statechart-methods). Fundamental statechart methods
...
h3(#generating_code_cpp_time-controlled-state-machines). Time-controlled state machines

If a statechart uses timing functionality, additional classes are generated.

The traffic light example uses timing funtionality, namely _after_ clauses. To support time-controlled behavior, the abstract classes _TimedStatemachineInterface_ and _TimerInterface_ are generated.
...
h4(#generating_code_cpp_function-settimer). Function setTimer
...
h4(#generating_code_cpp_function-unsettimer). Function unsetTimer
...
h4(#generating_code_cpp_function-raisetimeevent). Function raiseTimeEvent
...
h3(#generating_code_cpp_accessing-interfaces-variables-and-events). Accessing interfaces, variables and events
...
h3(#generating_code_cpp_operation-callbacks). Operation callbacks
...
h1(#generating_code_java_code-generator). Java code generator

h2(#generating_code_java_generator_features). Java generator features

==<!-- Start sgen_feature_naming -->==

h3(#generating_code_java_naming_feature). Naming feature
...
h3(#generating_code_java_synchronizedwrapper_feature). SynchronizedWrapper feature
...
h3(#generating_code_java_runnablewrapper_feature). RunnableWrapper feature
...
h3(#generating_code_java_GeneralFeatures). GeneralFeatures feature
...
h2(#generating_code_java_specification-of-java-code). Specification of Java code

In order to describe the API specifications of the code generated by the YAKINDU Java generator, the explanations below are using the _Traffic Light_ sample state machine, see "figure &quot;The _Traffic Light_ sample statechart model&quot;":#generating_code_java_fig_the_traffic_light_sample_statechart_model. It models a pedestrian crossing with push-button operated traffic lights ("pelican crossing").

p(#generating_code_java_fig_the_traffic_light_sample_statechart_model). 
!(full)images/docu_TrafficLight.png(The _Traffic Light_ sample statechart model)!
...
h3(#generating_code_java_generated-code-files). Generated code files

Generally you will find generated code at the places specified in the SGen model, see "section &quot;Outlet&quot;":#generating_code_outlet_feature for details.
...
h3(#generating_code_java_the-state-machine-interface). The state machine interface
...
h4(#generating_code_java_fundamental-statechart-methods). Fundamental statechart methods
...
h3(#generating_code_java_time-controlled-state-machines). Time-controlled state machines

If a statechart uses timing functionality, additional classes are generated.

The traffic light example uses timing funtionality, namely _after_ clauses. To support time-controlled behavior, the interfaces _ITimerCallback_ and _ITimer_ are generated. Like _IStatemachine_, they are independend of any particular state machine and are generated in the _libraryTargetFolder_ directory, if specified. See "section &quot;Outlet&quot;":#generating_code_outlet_feature for details.
...
h3(#generating_code_java_default-timer-implementation). Default timer implementation
...
h3(#generating_code_java_timer-service). Timer service
...
If suitable, an application can use the default timer service class _TimerService_, see "section &quot;Default timer implementation&quot;":#generating_code_java_default-timer-implementation for details.
...
h4(#generating_code_java_method-settimer). Method setTimer
...
h4(#generating_code_java_method-unsettimer). Method unsetTimer
...
h3(#generating_code_java_raising-time-events-on-a-state-machine). Raising time events on a state machine
...
h4(#generating_code_java_method-timeelapsed). Method timeElapsed
...
h3(#generating_code_java_runtime-service). Runtime service
...
bq. *Please note:* To enable this feature, set the _RuntimeService_ parameter of *GeneralFeatures* to _true_, see "Java: General Features":#generating_code_java_GeneralFeatures. 



h3(#generating_code_java_accessing-interfaces-variables-and-events). Accessing interfaces, variables and events
...
h3(#generating_code_java_interface-observers). Interface observers
...
h3(#generating_code_java_operation-callback). Operation callbacks
...
h3(#generating_code_java_integrating-generated-code). Integrating generated code
...
h3(#generating_code_java_simulating-operations-with-custom-java-code). Simulating operations with custom Java code
...
h1(#generating_code_custom-code-generators). Custom code generators
...
h2(#generating_code_prerequisits). Prerequisits
...
h2(#generating_code_creating-a-new-custom-code-generator-project). Creating a new custom code generator project
...
h3(#generating_code_executing-the-custom-code-generator). Executing the custom code generator
...
After that, create a new generator model as described in section "&quot;Configuring a generator&quot;":#generating_code_configuring-a-generator. Select _Custom Xtend2/Java based code generator_ as the generator to use. As you may have noticed, the generator model for the @yakindu::generic@ generator contains an additional feature called _Generator_. This is where you can specify the name of your custom generator project and the fully-qualified generator class name as shown below. 
...
h3(#generating_code_sgen_features-for-custom-code-generators). SGen features for custom code generators

==<!-- Start sgen_feature_generator -->==

h4(#generating_code_sgen-generator_feature). Generator feature
...
h2(#generating_code_different-meta-models-for-different-use-cases). Different meta models for different use cases
