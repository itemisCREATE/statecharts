...
A test class consists of the keyword @testclass@, followed by the name of the test class, followed by the keywords @for statechart@, followed by the name of the statechart this test class relates to.
...
The _body_ of an operation is enclosed in braces (<code>{ … }</code>). It consists of a sequence of "statements":#sctunit_statements_and_expressions, which may be empty. If the operation specifies a return type different from @void@, the operation must return a value of that type, using the "_return_ statement":#sctunit_returning_an_operations_result.
...
Operations that are declared in the statechart _cannot_ be called from a testclass.
...
The third assertion checks whether the operation _myOperation_ has been called exactly 4 times, no matter the arguments, while the fourth assertion checks if the operation has been called exactly one time with the specified parameters.
...
The _raise_ statement raises one of the state machine's _incoming_ events.
...
Since state machine internals are inaccessible to SCTUnit tests and outgoing events cannot be raised in general, only incoming events can be raised. Internal events, defined in the internal scope, and all outgoing events defined in an interface cannot be raised.
...
When running an SCTUnit test, the state machine does not run in real time, but in virtual time. That is, a statement like @proceed 3600 s@ does not have to wait for one hour of real time to elapse. Instead the state machine "leaps" by one hour in an instant, raises all affected time events, and processes them.
...
The _mock_ statement allows you to mock the operations defined in the statechart. You can specify what should be returned when the operation is called during execution of the test, even depending on the given input parameters.

==<div class="example">==

bc. 
mock myOperation returns (3.14)
mock myOperation(2) returns (6.28)
mock myOperation returns (d)

==</div>==

These are some usage examples - you can specify a default return value, and you can create mappings from parameter values to return values. You can create as many mappings as you like. Furthermore, you can specify that the mock should return a certain variable. When the variable changes, the return value is changed as well.

You can also override mappings - when a mocking call specifies parameter values (or none) that were specified before, the mapping is replaced. In the example above, the default return value is changed from @3.14@ to @d@.
...
The first step is to create a _generation model (SGen)_ for the SCTUnit test, similar to explanation in section "&quot;Generating state machine code&quot;":../user-guide/code_generation_custom.html#setting_up_a_code_generator_project.
