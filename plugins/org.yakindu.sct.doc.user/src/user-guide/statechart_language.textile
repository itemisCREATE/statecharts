...
h1(#statechart_language_the_statechart_language). The statechart language
...
h2(#statechart_language_state_machine_basics). State machine basics

h3(#statechart_language_words_and_definitions). Words and definitions

A "*finite-state machine (FSM)*":https://en.wikipedia.org/wiki/Finite-state_machine or *finite-state automaton (FSA, plural: automata)*, *finite automaton*, or simply a *state machine* as a mathematical model of computation. It is an abstract machine that can be in exactly one of a finite number of "states":#statechart_language_states at any given time. The FSM can change from one state to another in response to some external inputs; the change from one state to another is called a "transition":#statechart_language_transitions. An FSM is defined by a list of its states, its initial state, and the conditions for each transition.

A *"state diagram":https://en.wikipedia.org/wiki/State_diagram* is one of many possible representations of a finite-state machine.

A *"statechart":https://en.wikipedia.org/wiki/State_diagram#Harel_statechart* or *Harel statechart* is an extended form of the classic state diagram. The basic principles are the same, however. In this documentation, the term _statechart_ usually denotes the _graphical representation_ of a finite-state machine.

The term *state machine* is a short form of _finite-state machine_. In this documentation, the term _state machine_ usually pertains to the _dynamic aspects_ of an finite-state machine, i.e. it is more thought of as being executed, simulated etc. than being depicted.



h3(#statechart_language_automata_theory). Automata theory
...
h4(#statechart_language_classic_state_diagrams). Classic state diagrams
...
h4(#statechart_language_moore_and_mealy_machines). Moore and Mealy machines
...
h4(#statechart_language_harel_statecharts). Harel statecharts
...
###. h4(#statechart_language_yakindu_statecharts_and_uml_statecharts). YAKINDU statecharts and UML statecharts
...
###. h2(#statechart_language_why-statechart-language). Why another language?
...
h2(#statechart_language_statechart_model). Statechart model

A *statechart* or *statechart model* consists of several components.

The most important part is the _canvas_, comprising one or more top-level "regions":#statechart_language_regions. Regions in turn are containing "states":#statechart_language_states and "transitions":#statechart_language_transitions. For hints on how to use the statechart editor to modify a statechart, see section "&quot;Editing statecharts&quot;":../user-guide/editing_statecharts.html.

The _definition section_ is a textual statechart model component. It contains definitions of "namespaces":#statechart_language_namespaces, "interfaces":#statechart_language_interface_scopes, "variables":#statechart_language_variables, "events":#statechart_language_events, "operations":#statechart_language_operations, etc.
...
A type system or _domain_ specifies which types are available. YAKINDU Statechart Tools has a built-in type system.  Section "&quot;Types&quot;":#statechart_language_types lists the types that are available in YAKINDU Statechart Tools' standard domain.
...
h2(#statechart_language_graphical_elements). Statechart elements

h3(#statechart_language_regions). Regions
...
Regions typically contain "states":#statechart_language_states and "transitions":#statechart_language_transitions. A state in turn can contain one or more regions, turning it into a "composite state":#statechart_language_composite_states (contains one region) or a "orthogonal state":#statechart_language_orthogonal_states (contains two or more regions).



h3(#statechart_language_states). States

*States* are the central elements of a state machine. A state has to be placed inside a "region":#statechart_language_regions and must have a name that is unique witin this region. During state machine execution, a state is either _active_ or _passive_.

A state can have _behavior_. The behavior specifies which "actions":#statechart_language_reactions are executed on which conditions. Such actions can be triggered by entering the state, leaving the state, occurrence of events, conditions becoming true, or time passing. The behavior specification is a text in a state's box and consists of a sequence of "_local reactions_":#statechart_language_reactions. 

==<div class="example">==

The state *Countdown* in the sample statechart below has three local reactions:

p(#statechart_language_fig_state_example). 
!images/docu_state_010_example_01.png(State example)!

p=. State example: A simple state machine to launch a rocket.

When the *Countdown* state is entered, it sets the counting variable _t_ to 10. As long as that variable's value is greater than 0, each second it gets counted down by 1. The operation _speak_ provides audible information on how many seconds to go until the rocket is launched. If _t_ has been counted down to 0 – checked by a transition's _guard condition_ –, the *Countdown* state will become inactive and launch the rocket upon exiting. The countdown can be aborted anytime before launch by a _cancel_ event. (See figure "&quot;Transition example&quot;":#statechart_language_fig_transition_example for a different state machine which solves the same task but puts more emphasis on transitions.)
...
h3(#statechart_language_transitions). Transitions

A *transition* is diagrammed as an arrow leading from a _source state_ to a _target state_. When a transition is taken, the source state ceases to be active, and the target state becomes active instead.

A transition should have a _"reaction":#statechart_language_reactions_. A transition's reaction specifies
...
The occurrence of an event, a condition becoming true, or time passing can trigger a transition. The transition's reaction is attached to the transition's arrow as a text. Its syntax can be found in the section on "reactions":#statechart_language_reactions.
...
p(#statechart_language_fig_transition_example). 
...
The initial transition's _effect_ sets the counting variable _t_ to 10. As long as that variable's value is greater than 0, every second it gets counted down by 1. The operation _speak_ – executed as one of a transition's _actions_ – cares for audible information on how many seconds to go until the rocket is launched. If _t_ has been counted down to 0 – checked by a transition's _guard condition_ –, the rocket is launched. The countdown can be aborted anytime before launch by a _cancel_ event which _triggers_ the appropriate transition to fire. (See figure "&quot;State example&quot;":#statechart_language_fig_state_example for a different state machine which solves the same task, but puts more emphasis on states.)

==</div>==

Figure "&quot;Transition syntax overview&quot;":#statechart_language_fig_transition_syntax_overview shows how the syntax of a transition reaction is defined.

p(#statechart_language_fig_transition_syntax_overview). 
!images/docu_grammar_030_transition-overview.png(Transition syntax overview)!

p=. Transition syntax overview


h4(#statechart_language_entry_and_exit_points). Entry and exit points
...
The syntax to connect a transition to an "entry point":#statechart_language_entry_points with the name _entry-point-1_ of a target composite state is as follows:

<code># ><em>entry-point-1</em></code>

A "named exit point":#statechart_language_exit_points can be used as follows:
...
The sample statechart in figure "&quot;Entry and exit points&quot;":#statechart_language_fig_state_entry_exit contains two composite states:
...
p(#statechart_language_fig_state_entry_exit). 
...
The entry or exit points to take have to be specified at the right-hand side of a transistion specification. The corresponding syntax element is the _TransitionProperties_ non-terminal symbol that is shown in figure "&quot;Transition syntax overview&quot;":#statechart_language_fig_transition_syntax_overview. It is part of the _TransitionReaction_.

More on entry and exit points can be found in sections "&quot;Entry point&quot;":#statechart_language_entry_points and "&quot;Exit point&quot;":#statechart_language_exit_points.


h4(#statechart_language_transition_priorities). Transition priorities
...
p(#statechart_language_fig_transition-priorities-1). 
...
p(#statechart_language_fig_transition-priorities-2). 
...
h3(#statechart_language_entry_points). Entry points

When a state machine starts or when the control flow enters a "region":#statechart_language_regions, an *entry point*, or simply *entry*, defines which "state":#statechart_language_states is to be activated first. The flow passes through the state machine's or the region's entry point and transitions to the target state the entry point's transition is pointing to. This state becomes active.
...
An entry point is depicted as a filled circle, see figure "&quot;Entry point, exit point, and final state&quot;":#statechart_language_fig_state_entry_exit_final_explained.

p(#statechart_language_fig_state_entry_exit_final_explained). 
...
h4(#statechart_language_default_entry_point_and_initial_state). Default entry point and initial state
...
An incoming transition to a composite state can specify the named entry point it shall lead to. For details, see section "&quot;Entry and Exit points&quot;":#statechart_language_entry_and_exit_points.

Named entry points have no equivalent in the "UML":https://en.wikipedia.org/wiki/Unified_Modeling_Language.

==<div class="example">==

The sample statechart in figure "&quot;Entry point&quot;":#statechart_language_fig_state_entry has a composite state named *Handle result*. This composite state has a default entry point as well as an entry point called _failure_. If state *A* is active and the _error_ trigger fires, control is transitioned to the *Handle result* composite state. The notation @error # >failure@ specifies that the _failure_ entry point is to be used; see section "&quot;Entry and exit points&quot;":#statechart_language_entry_and_exit_points.

p(#statechart_language_fig_state_entry). 
...
h3(#statechart_language_exit_points). Exit points

An *exit point*, or simply *exit*, is a pseudo state that is used to leave and deactivate a composite state. Exit points are counterpart to "entry point":#statechart_language_entry_points. See section "&quot;Final state&quot;":#statechart_language_final_state for a different way to terminate a region or state machine.

An exit point may have multiple incoming transitions and has no outgoing one.

An exit point is depicted as an unfilled circle with an X-shaped cross, see figure "&quot;Entry point, exit point, and final state&quot;":#statechart_language_fig_state_entry_exit_final_explained.

Within a region, multiple exit points are allowed. Each exit point must either have a name that is unique within its region or be the _default exit point_. The default exit point is either unnamed or has the name _default_, which is semantically equivalent. A region may have at most one default exit point. For details on how to connect named exit points to transitions, see section "&quot;Entry and exit points&quot;":#statechart_language_entry_and_exit_points.

When the control flow of a composite state reaches an exit point in any of its regions, all states in other regions of that composite state, if any, are deactivated immediately. The composite state will be left and be deactivated. It maintains no status information that could be probed from the outside. In other words, reaching an exit point in one of a composite state's regions has severe consequences for all the other regions, since they are exited and disposed immediately. After that, the containing composite state is also exited. There must be an unguarded transition that catches the exit and leads to another state outside of the composite state. The semantics of an exit point is different from that of a final state; please see section "&quot;Final state&quot;":#statechart_language_final_state for details.

Named exit points have no equivalent in the "UML":https://en.wikipedia.org/wiki/Unified_Modeling_Language.



h3(#statechart_language_final_state). Final state

A *final state* denotes the end of the execution flow of a state machine or region. See section "&quot;Exit point&quot;":#statechart_language_exit_points for a different way to terminate a composite state.

A final state is depicted as an unfilled circle with a smaller filled black circle inside, see figure "&quot;Entry point, exit point, and final state&quot;":#statechart_language_fig_state_entry_exit_final_explained.
...
When a region reaches its final state, control stops there and waits until all other orthogonal regions, if any, have reached their respective final states, too. The semantics of final states is different from that of exits; please see section "&quot;Exit point&quot;":#statechart_language_exit_points for details.
...
h3(#statechart_language_choices). Choices
...
h3(#statechart_language_synchronizations). Synchronizations
...
* A synchronization with a single incoming transition and several outgoing ones _forks_ the flow into several regions of the same substate. These regions are executed in parallel, see section "&quot;Orthogonal states&quot;":#statechart_language_orthogonal_states.
...
A synchronization state is shown as a thick horizontal or vertical line, as can be seen in figure "&quot;Synchronization state&quot;":#statechart_language_fig_state_synchronization.
...
h3(#statechart_language_composite_states). Composite states

A *composite state* is a "state":#statechart_language_states containing a "region":#statechart_language_regions. All usual characteristics of regions apply - they can have entry points, exit points, and so on. "Transitions":#statechart_language_transitions can connect inner states of composite states to states that are at the same hierarchical level as the composite state, or they can directly be sourced at the composite states. This is useful for constructs like "cancel", where the operation of a composite state should be able to be aborted every time, no matter the active substate.

See the sections on "entry point":#statechart_language_entry_points, "exit point":#statechart_language_exit_points, "final state":#statechart_language_final_state and "connecting entry points and exit points to transitions":#statechart_language_entry_and_exit_points for further information and examples.



h3(#statechart_language_orthogonal_states). Orthogonal states

An *orthogonal state* is basically a "composite state":#statechart_language_composite_states with more than one region. These regions are executed virtually concurrently. *Please note* the word _virtually_! YAKINDU Statechart Tools does not guarantee in any way that orthogonal regions are _really_ executed concurrently. At the moment, no code generator utilizes threads to achieve this. Orthogonal states should rather be understood as a manner to have two or more sub-statecharts working together, however, they are executed _one after the other other_ in every cycle, and in a defined order: top to bottom, left to right. The same applies to multiple regions in the statechart itself. Please consult section "&quot;Raising and processing an event&quot;":#statechart_language_raising_and_processing_an_event for further information on region priorities and their meanings for the statechart execution.
...
h3(#statechart_language_shallow_history_states). Shallow history states
...
A history state saves the current state exactly when its containing region is left. A shallow history state saves the state of the current level only. If you need to save the status of multiple nested composite states, use a "deep history state":#statechart_language_deep_history_states.
...
h3(#statechart_language_deep_history_states). Deep history states
...
A history state saves the current state exactly when its containing region is left. A deep history state saves the status of multiple nested composite states. If saving the state of the current level only is sufficient, consider using a "shallow history state":#statechart_language_shallow_history_states instead.




h2(#statechart_language_definition_section). Definition section
...
The *definition section* is a text area. Per default, it is located to the left of the "canvas":../user-guide/editing_statecharts.html#canvas. You can move or resize it like other elements on the canvas.

In the definition section, you have to define entities you want to use in your statechart. This includes "variables":#statechart_language_variables, "events":#statechart_language_events, and "operations":#statechart_language_operations. Variables and events have to be defined in the scope of a "named interface":#statechart_language_interface_scopes or the "internal interface":#statechart_language_internal_scope. Especially useful for larger parts of a statechart model is the possibility to use "namespaces":#statechart_language_namespaces.
...
h3(#statechart_language_scopes). Scopes

==<!-- Start stext_keyword_namespace -->==

h4(#statechart_language_namespaces). Namespaces
...
h4(#statechart_language_interface_scopes). Interface scopes
...
bq. *Please note:* All elements defined in a named interface _must_ be referenced using that interface's name, such as @NamedInterface.event1@, @NamedInterface.event3@, or @NamedInterface.variable1@.
...
h4(#statechart_language_internal_scope). Internal scope
...
h3(#statechart_language_variables). Variables

*Variables* have to be defined in an "internal":#statechart_language_internal_scope or "external":#statechart_language_interface_scopes interface. Variables in the internal scope are not visible to any client code.

bc. var variable1: real

Variables that are defined in a named interface have to be referenced using the interface name, see section "&quot;Interfaces&quot;":#statechart_language_interface_scopes.

Variables are always typed, see section "&quot;Types&quot;":#statechart_language_types.
...
h3(#statechart_language_constants). Constants
...
h3(#statechart_language_events). Events
...
* *External events* are defined in an "interface scope":#statechart_language_interface_scopes and are either *incoming* or *outgoing*.

Events that are defined in a named interface need to be referenced using the interface's name, see section "&quot;Interfaces&quot;":#statechart_language_interface_scopes.

Events can be processed in triggers, see section "&quot;Trigger specification&quot;":#statechart_language_trigger_specifications. In order to raise an event in the state machine, see section "&quot;Raising an event&quot;":#statechart_language_raising_an_event. For details on the processing of events see section "&quot;Raising and processing an event&quot;":#statechart_language_raising_and_processing_an_event.


h4(#statechart_language_incoming_and_outgoing_events). Incoming and outgoing events
...
h4(#statechart_language_events_with_variables). Events with values
...
Read access to an event's value is possible in the statechart using the _valueof()_ built-in method, see section "&quot;Built-in methods&quot;":#statechart_language_built-in-functions for details.
...
Regarding the syntax of raising an event, see section "&quot;Raising an event&quot;":#statechart_language_raising_an_event. Regarding the more complicated details of processing an event, see section "&quot;Raising and processing an event&quot;":#statechart_language_raising_and_processing_an_event.
...
h3(#statechart_language_operations). Operations
...
Operations can have none, one or multiple parameters. A parameter is declared with a name and a type. An operation may have a single or no return type similar to usual programming languages. Please see section "&quot;Types&quot;":#statechart_language_types for details.
...
h2(#statechart_language_reactions). Reactions
...
h3(#statechart_language_trigger_specifications). Trigger specifications

Reactions can define a *trigger* that needs to occur to activate the reaction. "Events":#statechart_language_events are possible triggers, as well as certain keywords defined in the statechart language.

Triggers can be combined with "_guards_":#statechart_language_guard_conditions to further define the execution conditions of the reaction. In that case, the reaction is only executed if both the trigger occurs and the guard condition is true.
...
* "@after@: execute after a given time":#statechart_language_reaction_trigger_after 
* "@every@: execute periodically after a given time":#statechart_language_reaction_trigger_every
* "@always@: execute always":#statechart_language_reaction_trigger_always
* "@oncycle@: same as @always@":#statechart_language_reaction_trigger_oncycle
* "@else@: useful for choice states":#statechart_language_reaction_trigger_else
* "@default@: same as @else@":#statechart_language_reaction_trigger_default
* "@entry@: execute upon entering the state":#statechart_language_reaction_trigger_entry
* "@exit@: execute upon exiting the state":#statechart_language_reaction_trigger_exit
...
Figure "&quot;Event specifications syntax overview&quot;":#statechart_language_fig_transition_syntax_overview shows where to put event specifications in a reaction trigger and how their syntax is defined.

p(#statechart_language_fig_event-specifications-syntax-overview). 
...
h4(#statechart_language_reaction_trigger_after). Reaction trigger "after"
...
h4(#statechart_language_reaction_trigger_every). Reaction trigger "every"
...
h4(#statechart_language_reaction_trigger_always). Reaction trigger "always"
...
h4(#statechart_language_reaction_trigger_oncycle). Reaction trigger "oncycle"
...
h4(#statechart_language_reaction_trigger_else). Reaction trigger "else"
...
h4(#statechart_language_reaction_trigger_default). Reaction trigger "default"
...
h4(#statechart_language_reaction_trigger_entry). Reaction trigger "entry"
...
h4(#statechart_language_reaction_trigger_exit). Reaction trigger "exit"
...
h3(#statechart_language_guard_conditions). Guard conditions

A *guard condition*, or simply *guard*, is another option that can be used to specify when a reaction should be executed. Guard conditions can be used on their own or in conjunction with a specified "_trigger_":#statechart_language_trigger_specifications. In the latter case, the trigger needs to be fired _and_ the guard needs to be true for the reaction to fire its effect.

As a reminder, the complete reaction syntax is as follows:

_trigger_ @[@ _guard_ @] /@ _effect_

In the square brackets you can use any "expression":#statechart_language_expressions that evaluates to a boolean. Boolean variables, all kinds of comparisons, and operations returning a boolean fall into this category.
...
Please consult figure "&quot;Guard condition syntax overview&quot;":#statechart_language_fig_guard-condition-syntax-overview to find out how a guard condition syntactically fits into a reaction.

p(#statechart_language_fig_guard-condition-syntax-overview). 
...
h3(#statechart_language_actions_and_reaction_effects). Actions and reaction effects
...
This means that the effect will only be executed if the trigger fires or does not exist _and_ the guard condition evaluates to true or does not exist _and_ at least a trigger or a guard exist. See the sections about "triggers":#statechart_language_trigger_specifications and "guard conditions":#statechart_language_guard_conditions.

Normally, actions are "statements":#statechart_language_statements. Note that you can also use the "conditional expression":#statechart_language_conditional_expression to make some constructs more elegant.
...
Figure "&quot;Reaction effect syntax&quot;":#statechart_language_fig_reaction-effect-syntax shows the syntax definition.

p(#statechart_language_fig_reaction-effect-syntax). 
...
h4(#statechart_language_raising_and_processing_an_event). Raising and processing an event

For the syntax of event raising, see section "&quot;Raising an event&quot;":#statechart_language_raising_an_event.
For the syntax of event declaration, see section "&quot;Events&quot;":#statechart_language_events.
...
If an event _e_ is raised in region _r<sub>m</sub>_, then _e_ is available only in those regions that have not been processed yet, i.&nbsp;e. in _r<sub>i</sub>_ where _i_ > _m_ and _i_ ≤ _n_.
 ...
 The following example illustrates this. When the state machine is started, the situation is as shown in figure "&quot;Raising an event &#91;1]&quot;":#statechart_language_fig_raising-an-event_1: *A* and *X* in the parallel regions _r1_ and _r2_ are active states. Important: Region _r1_ is executed *before* _r2_.

p(#statechart_language_fig_raising-an-event_1). 
!images/docu_transition_010_raise_event_01.png(Raising an event [1])!

p=. Raising an event &#91;1]

Now event _e1_ occurs. It triggers transition _A → B_ in _r1_. This transition's effect @raise e2@ causes event _e2_ to be raised. This event triggers transition _X → Y_ in _r2_ to fire _within the same RTC_.

After that, the situation is now as can be seen in figure "&quot;Raising an event &#91;2]&quot;":#statechart_language_fig_raising-an-event_2: substates *B* and *Y* are now active states. However, the transition _B → C_ will not be taken, because that would have to happen in another RTC. But by then the particular _e2_ instance raised by _A → B_ will be long gone, since it was restricted to the RTC it has been raised in.

p(#statechart_language_fig_raising-an-event_2). 
...
The region priorities of the example above are shown in figure "&quot;Region priorities&quot;":#statechart_language_fig_region-priorities. Change the execution order and the example will behave differently than described.

p(#statechart_language_fig_region-priorities). 
...
h2(#statechart_language_types). Types
...
h2(#statechart_language_statements). Statements
...
h3(#statechart_language_assignments). Assignments
...
h3(#statechart_language_raising_an_event). Raising an event
...
See also sections "&quot;Declaring events&quot;":#statechart_language_events and "&quot;Raising and processing an event&quot;":#statechart_language_raising_and_processing_an_event.



h3(#statechart_language_calling_an_operation). Calling an operation

An operation is called similarly to other programming languages using the operation name and passing actual parameter values in parenthesis and separated by comma. The parameters can be expressions. The parenthesis can be omitted when no parameters are passed. Please see section "&quot;Operations&quot;":#statechart_language_operations for an example.




h2(#statechart_language_expressions). Expressions
...
h3(#statechart_language_logical_and). Logical AND

bc. expr1 && expr2



h3(#statechart_language_logical_or). Logical OR

bc. expr1 ||  expr2



h3(#statechart_language_logical_not). Logical NOT

bc. !expr1



h3(#statechart_language_conditional_expression). Conditional expression
...
h3(#statechart_language_bitwise_xor). Bitwise XOR

bc. expr1 ^ expr2



h3(#statechart_language_bitwise_or). Bitwise OR

bc. expr1 | expr2



h3(#statechart_language_bitwise_and). Bitwise AND

bc. expr1 & expr2



h3(#statechart_language_logical_relations_and_shift_operators). Logical relations and shift operators
...
h3(#statechart_language_binary_arithmetic_operators). Binary arithmetic operators
...
h3(#statechart_language_unary_arithmetic_operators). Unary arithmetic operators
...
h2(#statechart_language_built-in-functions). Built-in functions

==<!-- Start stext_keyword_valueof -->==

h3(#statechart_language_valueof_event). valueof(event)
...
h3(#statechart_language_active_state). active(state)
...
h2(#statechart_language_complete_statechart_language_grammar). Complete statechart language grammar
...
The grammar is visualized using "railroad diagrams":https://en.wikipedia.org/wiki/Syntax_diagram. Railroad diagrams are easy to read and comprise all syntactically valid language constructs. Figure "&quot;Railroad diagrams explained&quot;":#statechart_language_fig_railroad_diagrams_explained shows an example:

p(#statechart_language_fig_railroad_diagrams_explained). 
...
h3(#statechart_language_statechart_grammar). Statechart grammar

Figure "&quot;Statechart grammar&quot;":#statechart_language_fig_statechart_grammar shows the statechart grammar. It makes use of the "expressions grammar":#statechart_language_expressions_grammar.

p(#statechart_language_fig_statechart_grammar). 
...
h3(#statechart_language_expressions_grammar). Expressions grammar

Figure "&quot;Expressions grammar&quot;":#statechart_language_fig_expressions_grammar shows the expressions grammar. It is used by the "statechart grammar":#statechart_language_statechart_grammar.

p(#statechart_language_fig_expressions_grammar). 
