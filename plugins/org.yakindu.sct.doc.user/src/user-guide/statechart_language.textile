...
h4(#sclang_superstep). @SuperStep

The <code>@SuperStep</code> annotation enables the _superstep_ semantic for a run-to-completion step. In contrast to a regular step, a superstep executes all valid subsequent state transitions. 
...
h5(#sclang_superstep_eventdriven). @SuperStep and @EventDriven
