...
h3(#the-statechart-language-basics-theory). Automaton theory
...
Harel's statecharts are an expansion to the classic state diagrams, which describe finite state machines (FSM) and only consist of the following things:
...
It is possible to transform those types into each other, but the set of states, the transitions and the output functions needs to be changed to achieve this.

Harel's statecharts expand these classic state diagrams, producing a representation that can't be described as easily and concise mathematically as the types above, but is much more readable and usable in day-to-day usage.

It adds the following things and possibilities:

* Input and output is possible everywhere
...
When an "event":#events occurs, it may impact transitions and states:
* The event may trigger "transitions":#transition to "fire". The transition's source state, which is the currently active state, becomes inactive, control is transferred along the transition's arrow to the transition's target state, and that state becomes active. See section "&quot;Transition&quot;":#transition for more details.
* The event may cause "local reactions":#local-reactions within active states to be executed. See section "&quot;State&quot;":#state for more details.

For a transition as well as for a state's local reaction you can specify events that should trigger the transition or local reaction to be executed. Multiple events, separated by comma, can also be specified. If multiple events are specified, the occurrence of a single one is sufficient to trigger the transition or local reaction.
...
|_. Event specification: |_. Prerequisites for the transition to fire or the local reaction to be executed: |
{vertical-align: top}. |{width: 15ex} @ev@ | Event _ev_ occured. |
{vertical-align: top}. | @ev_1, ev_2@ | At least one of the events _ev_1_ and _ev_2_ occured. |
{vertical-align: top}. | @after 3s@ | Three seconds after activating the source state have passed. |
{vertical-align: top}. | @after 3s, ev@ | Three seconds after activating the source state have passed or event _ev_ occurred, whatever happens first. |
{vertical-align: top}. | @every 3s@ | Three seconds have passed. This event first occurs three seconds after activating the source state and then repeatedly every three seconds. – Within a state, the @every@ event specification is useful to execute a certain "reaction effect":#reaction-effect repeatedly. In a transition reaction without a "guard condition":#guard-condition, however, @every@ behaves just like @after@, because the transition is taken after the specified intervall and the source state becomes passive anyway. However, if the transition has a "guard condition":#guard-condition, @after@ checks it only once, while @every@ checks it periodically according to the specified interval time. |
{vertical-align: top}. | @always@ | Transition always fires; local reaction is always executed. (Watch out for what you are doing when using @always@ on a local reaction!) |
...
A _guard_ applies an additional condition for a "transition":#transition to fire or a "local reaction":#local-reactions to be executed. It is not sufficient for a specified event to occur, the guard condition must be fulfilled, too. It is written in square brackets following the "event specifications":#event-specifications. You can specify a guard condition without any event specification. In this case the behavior of the transition or the local reaction is as if the @always@ event had been specified. That is, the transition or local reaction are always executed, as long as the guard condition evaluates to _true_.

The following examples assume the _x_ is an integer variable and _isOpen_ is an operation.

|_. Transition specification: |_. Prerequisites for the transition to fire: |
{vertical-align: top}. |{width: 37ex}  @ev [x == 3]@ | Event _ev_ occured and _x_ holds the value 3. |
{vertical-align: top}. | @ev_1, ev_2 [x == 3]@ | At least one of the events _ev_1_ and _ev_2_ occured and _x_ holds the value 3. |
{vertical-align: top}. | @after 3s, ev [x == 3 && isOpen(7)]@ | Three seconds have passed or event _ev_ occurred, whatever happens first. Additionally, _x_ holds the value 3 and _isOpen(7)_ returns _true_. |
{vertical-align: top}. | @exit [isCanceled]@ | The active state is about to get passivated and the boolean variable _isCanceled_ is _true_. |

p. 

Please note that the guard condition applies to the whole event trigger, not just to the event specification left from it. That is, the guard condition must be fulfilled regardless of which of the events occured. You might think of the event specifications being enclosed in parentheses, like @(ev_1, ev_2) [x == 3]@, although it would be illegal to actually write these parentheses.

It is not possible to have event-specific guards as in (the illegal) @ev_1 [x == 1], ev_2 [x == 2]@. If you need something like this, use multiple transitions or multiple local reactions with @ev_1 [x == 1]@ on one of them and @ev_2 [x == 2]@ on the other.

Please consult figure "&quot;Guard condition syntax overview&quot;":#fig_guard-condition-syntax-overview to find out how a guard condition syntactically fits into a reaction trigger.
...
A transition specification may contain one or more _actions_ that are executed when the transition fires, like assigning a value to a variable, calling an operation, or raising an event. All actions taken together are called the transition's _reaction effect_.

A local reaction always has a _reaction effect_ with at least one action.

The reaction effect is written right from "event specifications":#event-specifications and "guard condition":#guard-condition. It is separated by a @/@ character from the former. Figure "&quot;Reaction effect syntax&quot;":#fig_reaction-effect-syntax shows the syntax definition.
...
When a transition or local reaction raises an event, please consider two important points:
...
h3(#interface-scope). Interface scope
...
h3(#reaction-trigger-after). Reaction trigger "after"
...
h3(#reaction-trigger-every). Reaction trigger "every"
...
h3(#reaction-trigger-always). Reaction trigger "always"
...
h3(#reaction-trigger-oncycle). Reaction trigger "oncycle"
...
h3(#reaction-trigger-else). Reaction trigger "else"
...
h3(#reaction-trigger-default). Reaction trigger "default"
...
h3(#reaction-trigger-entry). Reaction trigger "entry"
...
h3(#reaction-trigger-exit). Reaction trigger "exit"
