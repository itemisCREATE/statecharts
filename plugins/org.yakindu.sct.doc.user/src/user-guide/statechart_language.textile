...
A statechart or statechart model or state machine model consists of several components.
...
h4. Raising and processing an event
...
The sample state machine in figure "&quot;Entries and exits&quot;":#fig_state_entry_exit contains two composite states:
...
Read access to the value of an event is possible in the state machine using the @valueof()@ built-in method. Please note that reading an event's value is possible only when the event actually occurs, for example in a guard:
