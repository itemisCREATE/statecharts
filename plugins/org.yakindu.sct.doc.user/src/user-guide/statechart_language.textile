...
Variables can be _readonly_. For this variable the keyword @readonly@ is used. They are only readable by client code, but can also be written from within the statechart:
...
A variable can be immutable, i.e., *constant*. For this special variable the keyword @const@ is used. These variables can not be written by client code or from within the statechart:
