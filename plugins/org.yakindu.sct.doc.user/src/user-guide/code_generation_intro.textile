...
* "Python":../user-guide/code_generation_generators.html#codegen_python_code_generator
...
###. CHECK: YSCT supports code generation for C, C++, Java, Python, and statechart images.
...
###. CHECK: YAKINDU Labs generators: Swift, TypeScript
...
# In the _project explorer_ view, right-click on your project. The context menu opens.
...
!images/sgen_generator.png(Selecting code generator and statecharts)!
...
* _targetFolder_ (String, required): The folder within the _target project_ to write the generated artifacts to. If a _library target folder_ (see below) is specified, only the model-dependent artifacts are generated in the _target folder_. All artifacts in this folder will be overwritten during re-generation.
...
h3(#codegen_generating_code_eventprocessing_feature). EventProcessing feature
...
The name/value pairs specified by the @-v@ option would override the corresponding default values of the properties in the generator model. In this case, the properties _version_ and _isBeta_ would be set to the values "2.0" and "false", respectively. The variables _projectName_ and _generateTimerService_ would maintain their default values as specified in the SGen file.
