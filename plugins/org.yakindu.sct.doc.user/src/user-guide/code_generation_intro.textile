...
The state machine maintains all active states in a one-dimensional _stateVector_ array. 
Technically, a composite state and its first child region share the same entry in the state vector, optimizing the latter's memory requirements.

Examples of generated code are in Java. Source code in C or C++ look different, of course, but follow the same principles or patterns.



h3(#initializing_a_state_machine). Initializing a state machine

Upon initialization, the state machine's _init_ method sets all entries in the "state vector":#the_state_vector to the _null state_, i.e., setting all regions to have no active state. After that, incoming and outgoing events are cleared, and all variables are set to their default values, which are .



h3. Entering and exiting a state machine

After initialization, a state machine can be invoked by calling its _enter_ method. This will start the execution of the state machines. This is actually done by delegating the work to the top-level regions. Each of them has an _enter sequence_, i.e. code that is executed when the region is entered.

The state machine calls the regions' enter sequence in a specific order. Each region has a "priority":../user-guide/statechart_language.html#sclang_regions_and_priorities, and the enter sequences are executed according to these priorities.

==<div class="example">==

Example: Let's assume we have a statechart with three top-level regions _r1_, _r2_, and _r3_. These regions have the priorities 1, 2, and 3, respectively. Then the code generator produces the following statements for entering the statechart:
...
When the state machine enters a region, it does so via the region's initial state. If the region is not a top-level region, it is also possible to enter the region by an entry point. Control is transferred to the state the initial state or entry point is pointing to.

Entering a region consists of two steps:
# Executing any actions that are associated with the transition rooting at from the initial state or entry point.
# Entering the state the initial state or entry point is pointing to by executing that state's enter sequence.

Exiting a region is done by executing the active state's exit sequence.



h3. Entering and exiting a state
...
A run cycle's first action is to clear any outgoing events. After that, it iterates over the "state vector":#the_state_vector, i.e., over all regions on the top level or in composite states. Each entry in the state vector represents the active state of a region. A region that currently has no active state is tagged with the special _null state_ in the state vector.
...
For example, state *B* in region _r1_ is mapped to _enum State_ member _r1_B_ in the generated code. Its _react_ method's name is _react_r1_B_, which is called in the state machine's switch statement:
...
Now what does a state's react method do? Basically, it consists of zero or more _if_ statements checking for certain conditions via _check methods_ and, if conditions are fulfilled, executing associated actions by calling _effect methods_.

Basically, a state's react method checks whether any transition leading away from that state can be triggered. If such a transition is found, it will be taken. If there is no matching transition, the react method checks for local reactions and executes the matching ones. After that, substates are considered. At least that's the logic flow in the parent-first execution scheme. Under the child-first scheme, however, substates are checked for matching transitions first.

==<div class="example">==

An an example, here's the react method of state *A* in region _r1_. It has three outgoing transistions and some local reactions.
...
Outsourcing the evaluation of the condition as a check method and code execution as an effect method makes the 
...
## Iterate over the child regions, and check their active states for actions to be performed. If 
...
* _SCTVERSION_ (String): the current version of YAKINDU Statechart Tools, for example, "3.2.0"
