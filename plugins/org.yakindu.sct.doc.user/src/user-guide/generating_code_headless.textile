...
h4(#hdls_troubleshooting_cannot_not_find_project_files). [ERROR] Cannot find any .project files

Upon execution _scc_ searches for _.project_ files which are used to properly interpret generation target locations as specified in the _sgen_ files. These _.project_ files are hidden files that contain some meta information about your projects in your YAKINDU Statechart Tools workspace. If _scc_ does not find these files you might have copied your models to another folder and missed to also copy these
 files.


h4(#hdls_troubleshooting_couldnt_resolve_reference_to_eobject). [ERROR] Couldn't resolve reference to EObject
