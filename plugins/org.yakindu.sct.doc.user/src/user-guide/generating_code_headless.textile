...
usage: scc [-d <path>] [-h] [-m <path(s)>] [-v <arg>]
 ...
 -m,--model <path(s)>   A list of comma separated relative/absolute paths to model(s) used during execution. If not set
                        the runtime value of baseDir is used.
 -v,--variables <arg>   A list of comma separated key=value pairs to override variable in sgen models. baseDir is used.
p. 
