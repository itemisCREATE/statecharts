...
* Fixed C code generation when _@CShortIdentifier_ setting is used. In version 3.5.3, this resulted in generation of duplicate react methods.
...
The C# code generator now also supports statecharts with event-driven execution semantics. The generated code now contains corresponding run-cycle calls when events are triggered as well as event queues to store incoming events.
