...
To get the possibility to use the previously defined struct we have to import the 'point.h' header:

bc. 
import: "point.h"

The content assist (@[Ctrl+Space]@) can be used to see which imports(/includes) are available.

After that we can declare a variable *pointA* of the _Point_ type defined above. In the statechart's definition section, enter the following text:
...
* the C basic standard types without an explicit import,
* the C99 types provided by including _stdint.h_,
* the self-defined _Point_ type by including _point.h_,
...
h2(#cdom_namespaces). Imports & Includes

h3(#cdom_importing-a-namespace). Importing a C header

YAKINDU Statechart Tools provides the possibility to directly use C header files within the statechart model and therefore saves time during development - especially during integration of your state machine with your c program. In general you can import (or include) all C header files which resides in the same project or headers provided via one of the CDT projects include paths. (see  C Project  → Properties → C/C++ General → Paths and Symbols) 

To import a C header go to the beginning of the definition section within your statechart, enter @import:@ and hit @[Ctrl+Space]@. The content assist now shows all C headers you can import, besides other syntactical elements that would be valid here. In our example one of the first includes provided by the content assist will be _point.h_ that is placed in _/Geometry/point.h_. Other imports shown are provided by one of the include paths of the CDT project. The following picture for example shows headers provided by a basic _cygwin_ toolchain.

!images/cdom_geometry_060_importing_namespace_010.png(Selecting a namespace to import)!

p=. Selecting a C header to import

If we had more than a single header file in the project, we would see them all. The content assist shows all header files in a project, including those in sub-directories, relatively to the current statechart.
...
h3(#cdom_differentiating-between-namespaces). Differentiating between C headers
...
As on the C side we would run into problems if we tried to use both _Point_ definitions simultaneously, in a statechart this is not possible also.

A statechart definition section using a two-dimensional *pointA* would look like this:
...
And a statechart definition section using a three-dimensional *pointB* would look like this:

bc. import: "three-d/point.h"
interface:
    var pointA: Point


The example above shows that pathnames of header files placed in the same project are provided relatively to the actual statechart. 

C header files coming from one of the include paths of the CDT project, will be provided by simple names:

bc. import: "stdio.h"

If you are unsure how to import C header files, you can always use the content assist to do the import for you.
