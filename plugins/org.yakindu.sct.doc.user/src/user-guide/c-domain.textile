...
Making your self-defined C types, structs, and unions available in your YAKINDU statecharts saves you a lot of time and hassle that would otherwise be needed to map data from your C-type variables to statechart variables and vice versa.


h2(#cdom_overview). What you can do with the deep C/C++ integration

The deep C/C++ integration allows you to use constructs declared in header files in your statechart. For C, this includes:

* Functions, including variadic parameters
* Structs, Unions and Enums contained in a typedef
* If a struct contains a function pointer, you can call it from SCT
* @#define@'d values, e.g. @#define ARRAYSIZE 20@
* Variables
* Arrays, if they are allocated in the header file
* Pointers
...
Things that are not supported currently:

* Structs, Unions and Enums _not_ contained in a typedef
* Accessing or modifying function pointer values in structs
* Usage of constructors and initializers, memory allocation in general

h2(#cdom_usage). How to use the deep C/C++ integration

Using the deep C/C++ integration is pretty straightforward:

# Your license must include the C domain
# You have to use a C-Domain statechart or switch the domain of one you already created
# You have to have a C/C++ CDT project

p. The subsequent sections will explain how to use the C integration in practice, using a sample project. In this example, we will define some geometry types like _Point_ or _Triangle_ in C header files and demonstrate how to make them available and use them in a statechart model. While this example will only cover C constructs, there is no extra work involved in using C++ features, except your project must be a C/C++ or a pure C++ project.
...
When closing the type specification in a pointer declaration with angle brackets, e.&nbsp;g. @pointer<pointer<int32_t> >@, the @>@ characters must be separated from each other by one or more white space characters. Writing e.&nbsp;g. @pointer<pointer<int32_t>>@ would result in an error. This restrictions pertains to the current release candidate of YAKINDU Statechart Tools PRO only and will be fixed in a later release.
...
p. By importing a header file containing this class definition, one or more variables with type @Point@ can be defined in the statechart:
...
p. Furthermore it is of course possible to access the public functions and fields of these variables, so x and y can be set and read from within a state's or a transition's reactions:
...
h2(#cdom_templates). C++ template classes and functions

h3(#cdom_template_classes). Template classes

C++ allows to create generic constructs by defining templates. For example, if it should be possible to have @Point@ objects with integer as well as floating point coordinates, one could write:
...
p. This produces a generic type in YAKINDU Statechart Tools, which then has to be specified in the specification section:
...
p. Instead of @int32_t@, @double@, @ComplexNumber@ or any other type could have been specified instead.

Of course, YAKINDU Statechart Tools verifies the usage of these objects, so with @int32_t@, calling @PointA.set_x(4.2)@ is forbidden.
...
p. These functions do not have to have their type parameter declared, instead SCT just infers if all arguments are compatible. Calling @max(4, 3.5)@ would be fine, and @T@ would be inferred to be @double@, whereas @max(4, true)@ is invalid, because integer types and boolean types are not compatible.

h2(#cdom_namespaces). Namespaces

In C++, things can be organized in namespaces. This is typically done with classes.
...
p. To use the class @Point@, one would have to write @Geo::Point@ in C++. In YAKINDU Statechart Tools, namespaces are reflected as "Packages". For each header file that you import, a package is created, and one more for each namespace.
...
p. Of course, namespaces can be nested, so if namespace Geo would be contained in Math, Point would be addressed with @Math.Geo.Point@.
