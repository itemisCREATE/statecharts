...
The C Domain offers one more feature: The annotation @ShortCIdentifiers@ changes the naming scheme used by the code generator and activates more checks in the statechart validation to ensure that no identifiers produced by the code generator are longer than 31 characters. 
...
# Your YAKINDU Statechart Tools license must include the C domain.
# You have to use a "C domain" statechart or change the domain of an existing statechart.
...
Classes contained in C++ header files are usable from statecharts as well. Supported within C++ projects are the default CDT fileextenions for C++ Header files: ".h", ".hh", "hpp", "hxx" and ".inc".
