...
p. The subsequent sections will explain how to use the C integration in practice, using a sample project. In this example, we will define some geometry types like _Point_ or _Triangle_ in C header files and demonstrate how to make them available and use them in a state machine model.
...
In the created header file we define a struct type named *Point*, which we will later use in a state machine. A (two-dimensional) point consists of an _x_ and a _y_ coordinate. We choose *int16_t* to represent a coordinate, i.&#8239;e. a 16-bit signed integer. The complete header file containing the struct definition looks like this:
...
h2(#cdom_using_a_c_struct_in_a_statechart). Using C types in a state machine

h3(#cdom_creating_a_statechart_model). Creating a state machine model

Let's create a state machine model now to make use of the C type _Point_ we have just defined.
...
# The new statechart model is created:<br/>!(standard-image)images/cdom_geometry_030_new_statechart_model_030.png(New state machine model)!

h3(#cdom_defining_a_c_type_variable). Defining a C-type variable in a state machine

Variables are defined in the definition section on the left-hand side of the state machine editor. Double-click into the section to edit it.
...
h3(#cdom_using_a_c_type_variable_in_a_statechart). Using a C-type variable in a state machine

A state machine variable with a C type can be used everywhere a "normal" state machine variable can be used.
...
The state machine below uses these variables in various places, i.&#8239;e. in transition actions, in internal actions, and in guard conditions.
...
On the C side we would run into problems if we tried to use both _Point_ definitions simultaneously. However, in a state machine this is possible, because each header file represents a different namespace.

A state machine definition section using a two-dimensional *pointA* and a three-dimensional *pointB* would look like this:
...
During a state machine model simulation full access to the C data structures is possible on all layers. The user can inspect them as well as modify them in the simulation view.
...
!(standard-image)images/cdom_geometry_080_simulation_010_statechart.png(The rectangle comparison state machine)!

p=. The rectangle comparison state machine
...
p. In order to simulate the state machine, right-click on the state machine file in the project explorer and select _Run As → Statechart Simulation_ from the context menu.
...
Code generation, i.&#8239;e. turning a state machine model into source code of a programming language, is explained in the section _Generating state machine code_. Therefore we won't go into the details here, but instead only put some emphasis on code generation specialties of the _Deep C Integration_.
