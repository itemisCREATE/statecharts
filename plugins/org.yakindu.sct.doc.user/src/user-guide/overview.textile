...
The basic building blocks of a state machine are states and transitions. A state is a situation of a system depending on previous inputs and causes a reaction on following inputs. Transitions define for which external input and under which condition a state change occurs.

Consider the simple state machine above. It consists of two states, *Off* and *On*. *Off* is the initial state, i.e. it is activated when the state machine is executed. The arrows between the states denote the possible state transitions. They define for which input a state change occurs. Here, the active state is changed from *Off* to *On* for the input *buttonpressed*, and back again to *Off* for the same input. 

bq. *Please note:* In automata theory an automaton reacts on inputs and produces outputs. There, the terms input and output are usually used for text inputs/outputs. Modern state machines use an extended definition of inputs and outputs. Inputs can be events like a button click or a time trigger while outputs can be all kinds of expressions or operation calls.
...
In the light switch example we can now merge all *Light On - ...* states into one *Light On* state and produce the output on a transition. Here we just increment the brightness value each time the transition is taken. We use the modulo expression to ensure the brightness value stays between 1 and 3.
...
Although Mealy machines can already reduce the number of required states, for complex systems such automatons get easily unmanageable. This is also what Prof David Harel realized and put in the following statement:
...
So basically "Harel statecharts":http://www.wisdom.weizmann.ac.il/~dharel/SCANNED.PAPERS/Statecharts.pdf are Mealy/Moore machines extended by further concepts that allow us to model complex systems in a practical manner.
...
Because only transitions can produce outputs, many actions need to be duplicated. For example, from each state there is a transition @OFF_pressed / brightness = 0@ to the *Off* state. These redundancies make this rather simple system already harder to read and maintain as necessary.
