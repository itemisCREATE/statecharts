...
# Open the directory folder of the Angular application by clicking on the _Directory_ button.<br/>!images/tsdomain_import2_dialog.png(Import Projects from File System or Archive dialog)!
# Click _Finish_. The project explorer should now look like this:<br/>!images/tsdomain_project_explorer.png(Project explorer)!
# Open the project's context menu and add the _YAKINDU SCT Nature_. !images/tsdomain_add_sct_nature.gif(Adding YAKINDU SCT Nature)!
...
The content assist opens and shows the TypeScript types available in the Angular TypeScript sample application.

p(#tsdom-fig-selecting-routerservice-using-content-assist). 
!images/tsdomain_var.png(Selecting RouterService using content assist)!

p=. Selecting _RouterService_ using content assist

In the content assist menu, select the _RouterService_ entry to complete the variable definition.

p(#tsdom-fig-variable-routerservice-defined-in-the-definition-section). 
!images/tsdomain_routerService_var.png(Variable _routerService_ defined in the definition section)!

p=. Variable _routerService_ defined in the definition section
...
# In the main menu, select _File → New → YAKINDU SCT → Code Generator Model_. The wizard opens.
...
###. FIXME (Rainer): In the above text, please specify the wizard's exact name, that is, not just "The wizard opens", but something like "The _Explore the Universe_ wizard opens".
...
p(#tsdom-fig-generated-hmi-behaviour). 
