...
In this example, we will use an Angular project and demonstrate how the TypeScript artifacts can be used in the statechart model. The example also shows
...
p(#tsdom-fig-hmimain2). 
!images/tsdomain_hmimain2.png(HMI main screen)!
...
h3(#tsdom-importing-the-sample-angular-2-project-into-eclipse). Importing the sample Angular project into Eclipse
...
# Open the directory folder of the Angular application by clicking on the _Directory_ button.<br/>!images/tsdomain_import2_dialog.png(Import dialog)!
# Click _Finish_. The project explorer should now look like this:<br/>!images/tsdomain_project_explorer.png(Project explorer)!
# Open the project's context menu and add the _YAKINDU SCT Nature_. !images/tsdomain_add_sct_nature.gif(Add Yakindu SCT nature)!

h3(#tsdom-creating-a-new-project-for-statechart-models). Creating a new project for statechart models

The next step is to create a statechart project that makes use of the Angular  project resp. of the TypeScript artifacts therein.
...
# The new statechart model is created. !images/tsdomain_create_statechart_model.png(Statechart model)!
...
The content assist opens and shows the TypeScript types available in the Angular TypeScript sample application.<br/>!images/tsdomain_var.png(Content assist)!

In the content assist menu, select the _RouterService_ entry to complete the variable definition.

p(#tsdom-fig-routerservice). 
!images/tsdomain_routerService_var.png(Definition section - routerService)!

p=. Definition section - routerService
...
Figure "Diagram Appearance":#tsdom-fig-sample-screen-changing-statechart-model show how this is modeled. When the specified time has elapsed, a transition takes place with _blankScreen_ as its origin and with _mainScreen_ as its target state.
...
# In the main menu, select _File → New → YAKINDU SCT → Code Generator Model_. The wizzard opens.
...
p(#tsdom-fig-genmodel-config-wizard). 
!images/tsdomain_genmodel_config.png(SCT generator model configuration wizzard)!

p=. SCT generator model configuration wizard

The result is an _.sgen_ file of the following format:

p(#tsdom-fig-sgen-model). 
!images/tsdomain_sgen.png(SCT generator model)!

p=. SCT generator model
...
p(#tsdom-fig-sgen-model2). 
!images/tsdomain_sgen2.png(SCT generator model example configuration)!

p=. SCT generator model example configuration
...
p(#tsdom-fig-genfolder). 
!images/tsdomain_genfolder.png(Generated file structure)!

p=. Generated file structure

h3(#tsdom-integrating-the-generated-typescript-statemachine-into-an-angular-2-application). Integrating the generated TypeScript statemachine into an Angular application
...
When you start the Angular application, it should behave like this:

p(#tsdom-fig-hmi-behaviour). 
!images/tsdomain_hmi.gif(Generated HMI behaviour)!

p=. Generated HMI behaviour
