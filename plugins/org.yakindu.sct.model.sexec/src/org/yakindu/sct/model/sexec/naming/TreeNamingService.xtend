...
class TreeNamingService implements INamingService {
	...
	private static class StringTreeNode {
		...
		def public List<StringTreeNode> getChildren()
		{
			return children;
		}
		...
		{
			var firstChar = Character.toString(s.charAt(0));
			var next = "";
			var newChild = true;
			
			next = s.substring(1); //cut first char
			
			if(next.length() == 0) //break recursion
			{
				return;
			}
			...
			{
				if(child.getData().equals(firstChar))
				{
					newChild = false;
					child.addString(next);
				}
			}
			if(newChild)
			{
				var newNode = new StringTreeNode(firstChar);
				addChild(newNode);
				newNode.addString(next);
			}
		}
}
